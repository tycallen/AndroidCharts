...
public class LineView extends View {
	...
	private ArrayList<ArrayList<String>> bottomTextList = new ArrayList<ArrayList<String>>();
	...
	// popup 컬러
	private int[] popupColorArray = { R.drawable.popup_red,
			R.drawable.popup_blue, R.drawable.popup_green };
	 ...
	 */
	public void setBottomTextList(ArrayList<ArrayList<String>> bottomTextList) {
		this.dataList = null;
		...
		String longestStr = "";
		bottomTextDescent = 0;
		for (ArrayList<String> col : bottomTextList) {
			for (String s : col) {
		...
		if (dataLists != null && !dataLists.isEmpty()) {
			...
			for (int k = 0; k < dataLists.size(); k++) {
				...
				for (int i = 0; i < dataLists.get(k).size(); i++) {
					...
					int y = yCoordinateList.get(verticalGridNum
			...
			for (Dot d : drawDotLists.get(k)) {
				if (showPopupType == SHOW_POPUPS_All)
					drawPopup(canvas, String.valueOf(d.data),
							d.setupPoint(tmpPoint), popupColorArray[k % 3]);
	...
	private void drawBackgroundLines(Canvas canvas) {
		...
		for (int i = 0; i < xCoordinateList.size(); i++) {
			canvas.drawLine(xCoordinateList.get(i), 0, xCoordinateList.get(i),
					mViewHeight - bottomTextTopMargin - bottomTextHeight
							- bottomTextDescent, paint);
		}
		...
		// draw bottom text
		System.out.println(bottomTextList);
		if (bottomTextList != null) {
			int j = 0;
			for (ArrayList<String> col : bottomTextList) {
				for (int i = 0; i < col.size(); i++) {
					canvas.drawText(col.get(i), sideLineLength
							+ backgroundGridWidth * j, mViewHeight
							- bottomTextDescent + i * 20, bottomTextPaint);
				}
				j++;
			}
		}
