...
public class LineView extends View {
    ...
    // private ArrayList<ArrayList<String>> bottomTextList = new
    // ArrayList<ArrayList<String>>();
    // private ArrayList<Spanned> bottomSpannedList = new ArrayList<Spanned>();
    ...
    private ChartDotOnclickListener chartDotOnclickListener;
    ...
    // public void setBottomSpannedList(ArrayList<Spanned> bottomTextList){
    // this.dataList = null;
    // this.bottomSpannedList = bottomTextList;
    //
    // Rect r = new Rect();
    // int longestWidth = 0;
    // Spanned longestStr = Html.fromHtml("");
    // bottomTextDescent = 0;
    // for(Spanned s:bottomTextList){
    // // bottomTextPaint.getTextBounds(s,0,s.length(),r);
    // if(bottomTextHeight<r.height()){
    // bottomTextHeight = r.height();
    // }
    ...
    private void refreshYCoordinateList(int verticalGridNum) {
        ...
        for (int i = 0; i < (verticalGridNum + 1); i++) {
            yCoordinateList
                    .add(topLineLength
                            + ((mViewHeight - topLineLength - bottomTextHeight - bottomTextTopMargin - bottomLineLength - bottomTextDescent)
                                    * i / (verticalGridNum)));
        }
        System.out.println(yCoordinateList.size());
    }
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        // 도트리스트를 추가한다.
                        drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(k).get(i), k, i));
                    } else {
                        // 도트리스트에 타겟을 설정한다.
                        // drawDotLists.get(k).set(i,
                        // drawDotLists.get(k).get(i).setTargetData(x, y,
                        // dataLists.get(k).get(i), k));
                        // drawDotLists.get(k).set(i,
                        drawDotLists.get(k).get(i).setTargetData(x, y, dataLists.get(k).get(i), k);
                    }
                }
    ...
    private void refreshTopLineLength(int verticalGridNum) {
        ...
        if ((mViewHeight - topLineLength - bottomTextHeight - bottomTextTopMargin) / (verticalGridNum + 2) < getPopupHeight()) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (showPopup && selectedDot != null) {
            drawPopup(canvas, String.valueOf(selectedDot.data), selectedDot.setupPoint(tmpPoint),
    ...
    private void drawPopup(Canvas canvas, String num, Point point, int PopupColor) {
        ...
        Rect r = new Rect(x - popupTextRect.width() / 2 - sidePadding, y - popupTextRect.height()
                - bottomTriangleHeight - popupTopPadding * 2 - popupBottomMargin, x + popupTextRect.width() / 2
                + sidePadding, y + popupTopPadding - popupBottomMargin);
    ...
    private int getPopupHeight() {
        ...
        Rect r = new Rect(-popupTextRect.width() / 2, -popupTextRect.height() - bottomTriangleHeight - popupTopPadding
                * 2 - popupBottomMargin, +popupTextRect.width() / 2, +popupTopPadding - popupBottomMargin);
        return r.height();
    }
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (int i = 0; i < drawDotLists.get(k).size() - 1; i++) {
                canvas.drawLine(drawDotLists.get(k).get(i).x, drawDotLists.get(k).get(i).y,
                        drawDotLists.get(k).get(i + 1).x, drawDotLists.get(k).get(i + 1).y, linePaint);
            }
        }
    }
    ...
    private void drawBackgroundLines(Canvas canvas) {
        ...
        // mViewHeight - bottomTextTopMargin
        // - bottomTextHeight - bottomTextDescent, paint);
        // }

        // draw dotted lines 水平线
        paint.setPathEffect(effects);
        Path dottedPath = new Path();
        // for (int i = 0; i < 5; i++) {
        // dottedPath.moveTo(20, i * getHeight() / 5);
        // dottedPath.lineTo(getWidth(), i * getHeight() / 5);
        // canvas.drawPath(dottedPath, paint);
        // }
        for (int i = 0; i < yCoordinateList.size(); i++) {
            if ((yCoordinateList.size() - 1 - i) % 20 == 0) {
                dottedPath.moveTo(35, yCoordinateList.get(i));
                dottedPath.lineTo(getWidth(), yCoordinateList.get(i));
                canvas.drawPath(dottedPath, paint);
                canvas.drawText(String.valueOf(yCoordinateList.size() - 1 - i), 15, yCoordinateList.get(i),
                        bottomTextPaint);
            }
        }
        // draw bottom text
        if (bottomTextList != null) {
            for (int i = 0; i < bottomTextList.size(); i++) {
                canvas.drawText(bottomTextList.get(i), sideLineLength + backgroundGridWidth * i, mViewHeight
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (pointToSelect != null) {
                ...
                pointToSelect = null;
                postInvalidate();
                if (null != chartDotOnclickListener) {
                    chartDotOnclickListener.ChartDotOnclick(selectedDot.index);
                }
            }
        }
    ...
    class Dot {
        ...
        int targetY;
        int linenumber;
        int index;// the index-th dot
        int velocity = MyUtils.dip2px(getContext(), 18);

        Dot(int x, int y, int targetX, int targetY, Integer data, int linenumber, int index) {
            this.x = x;
            this.y = y;
    ...
    public interface ChartDotOnclickListener {
        void ChartDotOnclick(int i);
    }

    public void setChartDotOnclickListener(ChartDotOnclickListener chartDotOnclickListener) {
        this.chartDotOnclickListener = chartDotOnclickListener;
    }
