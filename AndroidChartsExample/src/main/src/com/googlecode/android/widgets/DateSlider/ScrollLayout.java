...
import com.dacer.androidchartsexample.R;
...
public class ScrollLayout extends LinearLayout {
    ...
    public ScrollLayout(Context context, AttributeSet attrs) {
        ...
        // values as defaults
        objWidth = a
                .getDimensionPixelSize(R.styleable.ScrollLayout_childWidth, mLabeler.getPreferredViewWidth(context));
    ...
    private void setTime(long time, int loops) {
        ...
        if (loops > 2 || currelem.getStartTime() <= time && currelem.getEndTime() >= time) {
            if (loops > 2) {
                Log.w("ABC", String.format("LOOPOVERLFOW: time: %d, start: %d, end: %d", time, currelem.getStartTime(),
                        currelem.getEndTime()));
                return;
            }
    ...
    protected void reScrollTo(int x, int y, boolean notify) {
        ...
        if (minTime != -1 && notify && scrollDiff < 0) {
            ...
            long esp_time = (long) (mCenterView.getStartTime() + (f - ((double) -scrollDiff) / objWidth)
                    * (mCenterView.getEndTime() - mCenterView.getStartTime()));

            // if we reach it, prevent surpassing it
            if (esp_time < minTime) {
                int deviation = scrollDiff
                        - (int) Math.round(((double) (currentTime - minTime)) / (currentTime - esp_time) * scrollDiff);
                mScrollX -= deviation;
                x -= deviation;
        ...
        // estimate whether we are going to reach the upper limit
            ...
            if (esp_time > maxTime) {
                int deviation = scrollDiff
                        - (int) Math.round(((double) (currentTime - maxTime)) / (currentTime - esp_time) * scrollDiff);
                mScrollX -= deviation;
                x -= deviation;
                scrollDiff -= deviation;
                if (!mScroller.isFinished())
        ...
        if (listener != null && notify) {
            ...
            int left = (getChildCount() / 2) * objWidth - scrollX;
            double f = (center - left) / objWidth;
            currentTime = (long) (mCenterView.getStartTime() + (mCenterView.getEndTime() - mCenterView.getStartTime())
                    * f);
